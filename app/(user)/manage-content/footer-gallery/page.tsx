...
import { getFooterImages, updateFooterImages } from "@/app/api/city/city";
...
const DynamicContent: React.FC = () => {
  ...
  const fetchImages = useCallback(async () => {
    setLoader(true);
    try {
      const response: any = await getFooterImages();
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await updateFooterImages(formData);
      if (response.remote === "success") {
  ...
  return (
    <Box>
      <Box mt={4}>
        <Typography variant="h3">Footer Images</Typography>
        <ImgCrop>
          <Upload
